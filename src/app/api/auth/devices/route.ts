...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = (await prisma.$queryRawUnsafe(
      `
        SELECT id, user_agent, expires_at::text, created_at::text, revoked_at::text,
               CASE WHEN id = $2 AND revoked_at IS NULL THEN true ELSE false END as is_current
          FROM user_sessions
         WHERE user_id = $1
...
export async function POST(req: NextRequest) {
  return NextResponse.json({ ok: false, error: '请使用 /api/local-auth/login 创建会话' }, { status: 405 });
}
